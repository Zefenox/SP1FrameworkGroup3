...
void init(void)
{
    ...
    // Enemy initial state (for now some hard coded - only works for array values)
    p1.m_bActive = true;
    p2.m_bActive = true;
    p3.m_bActive = true;
    p4.m_bActive = true;
    ...
    p5.m_cLocation.Y = 60;
    setStalkerCoords(stalkers); // stalker spawn
    bossBodyCoord(bossParticles, 150, 50); // boss spawn
...
char stalkerSearchPlayer(SGameChar EArr[10])
{
    ...
    if(stalkerSearchPlayer(stalkers) != 'n')
        ...
        case 'j':
            if ((player->getX() - 5 <= stalkers[9].m_cLocation.X) ||
                ...
                {
                    stalkers[9].m_cLocation.X++;
                }
                else if (player->getX() < stalkers[9].m_cLocation.X)
                {
                    stalkers[9].m_cLocation.X--;
                }
...
void renderBoss(SGameChar BArr[9])
{
    ...
    for (int i = 0; i < 9; i++)
    {
        if (BArr[i].m_bActive == true)
        {
            ...
            {
                g_Console.writeToBuffer(BArr[i].m_cLocation, (char)68, bossColor);
            }
            else
            {
                g_Console.writeToBuffer(BArr[i].m_cLocation, (char)33, bossColor);
